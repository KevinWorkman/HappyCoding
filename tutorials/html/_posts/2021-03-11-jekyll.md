...
- Use [includes](https://jekyllrb.com/docs/includes/) for content that's used in multiple places.
...
Another example of a cool thing you can do with Jekyll: check out the [site.json](/api/site.json) file. This file contains data about every post on Happy Coding, and I use it to build things like the [tags](/tags) and [stats](/about/stats) pages. And the whole `site.json` file is built using Jekyll! Check out the code [Github - KevinWorkman-HappyCoding-api](https://github.com/KevinWorkman/HappyCoding/blob/gh-pages/api/site.json). So you aren't limited to only creating HTML files with Jekyll- you can also create JavaScript, CSS, and any other text file.
