...
No matter how you download the example project, you'll end up with an `app-engine-hello-world` directory with subdirectories that contain 4 files:
...
- `app-engine-hello-world/`
  ...
  - `src/main/` is a directory that contains the code.
    ...
    - `WEB-INF/` is a directory that contains files that users cannot access directly, like config files.
      - **`appengine-web.xml`** is a config file with some App Engine settings.
...
This tells Maven to use the [App Engine Plugin](https://cloud.google.com/appengine/docs/standard/java/using-maven) defined in the `pom.xml` file to deploy the project to a **local server**. In other words, this turns your computer into a server that runs the code. This is very helpful for testing out code during development, before you deploy it to your live server. You can experiment without worrying about breaking your web app for any users.
...
[lhttp://localhost:8080](http://localhost:8080)
...
Bring your server down by pressing `ctrl+c` in your command line. Try making a simple change to the `index.html` and `HelloWorldServlet` files and running the server again to see your change.
...
When this command completes, navigate to `YOUR_PROJECT_ID_HERE.appspot.com` to view the live version of your project. You can also send this URL to [other people](https://forum.happycoding.io) for them to see your work.
