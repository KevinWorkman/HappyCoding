...
forumExcerpt: I posted a new article on the use of maps in technical interviews.
---
...
- `TreeMap` maintains the _natural ordering_ of its keys. In other words, if you call `keySet()` and then iterate over the keys, they'll be in alphabetical order for strings and numerical order for numbers.

# Algorithmic Complexity

The power of `Map` data structures come from the fact that `put(key, value)`, `get(key)`, and `containsKey()` are very efficient. For `HashMap` and `LinkedHashMap`, they're constant time `O(1)`, and for `TreeMap`, they're `log(n)`.
...
However, the code relies on the `ArrayList#contains()` and `ArrayList#indexOf()`, both of which are `O(n)`. And because the code calls these function inside a for loop, that means the algorithmic complexity is still `O(n ^ 2)`.
