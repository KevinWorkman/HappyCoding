...
Let's say have an array of `int` values, and you want to find the subarray of a given length that contains the highest sum of numbers. For example, for the array `{1, 5, 10, 7, -1}` and a length of `3`, the subarray of length `3` with the largest sum is `{5, 10, 7}` for a total of `22`.

You could do this using a nested for loop:

```java
int getMaxSubarray(int[] array, int subarrayLength){
  int maxSubarray = Integer.MIN_VALUE;

  // Loop over every start index, leaving room for the subarray at the end
  for(int startIndex = 0; startIndex <= array.length - subarrayLength; startIndex++){

    // Calculate the total of the subarray starting at startIndex
    int totalAtStartIndex = 0;

    // Iterate over every element in the subarray at startIndex
    for(int currentIndex = startIndex; currentIndex < startIndex + subarrayLength; currentIndex++){
      totalAtStartIndex += array[currentIndex];
    }

    if(totalAtStartIndex > maxSubarray){
      maxSubarray = totalAtStartIndex;
    }
  }
...
This code contains a for loop that iterates over every index in the array, and inside of that, it nests another for loop that iterates over the next `subarrayLength` indexes. In other words, it has an algorithmic complexity of `O(n * subarrayLength)`. Can that be improved?

Consider this example array, with a `subarrayLength` of `5`:
...
Now, no matter how large the `subarrayLength` is, each iteration only does `2` calculations: subtracting the leftmost element from the window, and adding the rightmost to the window.

Putting it into code:

```java
int getMaxSubarraySum(int[] array, int subarrayLength){
  ...
  int maxSubarraySum = windowSum;

  // Loop over every start index, leaving room for the subarray at the end
  for(int startIndex = 1; startIndex <= array.length - subarrayLength; startIndex++){
    ...
    windowSum += array[startIndex + subarrayLength - 1];
...
Now the code has two for loops, but they aren't nested! The complexity is now `O(subarrayLength + n)`, which is a big improvement.
