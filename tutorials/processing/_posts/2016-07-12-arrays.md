...
This program uses a `circleY` variable to show a circle falling down the screen. Incrementing the `circleY` each frame causes it to fall. The `if` statement detects when the circle reaches the bottom, and resets the circle back to the top of the screen.
...
Putting it all together, you could rewrite the program to use arrays instead of single-value variables:
...
In other words, you always want to stop the loop when its loop variable equals the number of elements in the array (also called the **length** of the array). If you try to access an index that's larger than the length, you'll get an error!

So if you add a variable to the array initialization (the values in the curly brackets `{}`), you'll have to change the check in the `for` loop. Wouldn't it be nice if the computer could keep track of that for us?
