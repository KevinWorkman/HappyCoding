...
Processing provides `mouseX` and `mouseY` variables that hold the current location of the mouse cursor in the window. Processing automatically updates these variables, so you can use them in the `drawm` function to get the position of the mouse.
...
Similarly, the `mousePressed` variable points to a `boolean` value that is `true` when the mouse is pressed, and `false` when it's not. You can use the `mousePressed` function in an `if` statement to do stuff when the mouse is held down:
...
Notice that the code only checks the `key` variable if it knows that `keyPressed` is true. This is because `key` always holds the **most recent** key pressed by the user, even if the user has stopped pressing it.
