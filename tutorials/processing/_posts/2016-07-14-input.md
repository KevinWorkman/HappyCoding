...
You can use the `keyPressed` variable in an `if` statement to do execute code whenever the user is pressing a key:
...
![green background when key is pressed](/tutorials/processing/input-9.gif)
...
![alphabet](/tutorials/processing/input-10.gif)
...
![key press changing background](/tutorials/processing/input-11.gif)
...
![arrow keys controlling circle](/tutorials/processing/input-12.gif)
...
![circle controlled by key release](/tutorials/processing/input-13.gif)
...
We can keep track of which keys are pressed
...
- Create a drawing program for a specific scene. For example: I might create a garden drawing program where clicking creates flowers (randomly sized and colored?), dragging creates grass, keyboard changes the background or sky or...
