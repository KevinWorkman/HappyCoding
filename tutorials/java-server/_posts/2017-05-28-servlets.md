...
- `index.html` is the name of the "file" that we request from our web app. But not that there isn't an actual `index.html` file anywhere! Instead, our `web.xml` file knows that any requests for `index.html` should be sent to our `HelloWorldServlet` class.
