...
The `status` variable holds the HTTP status code of the request. For example, it's `200` when the URL is loaded correctly, `404` if the file is missing (you've probably seen a `404` error while browsing the internet before), or `500` if the server had an error. [List of HTTP status codes](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) is a full list of possible values, but for now we probably only care about a `status` of `200` (success).
...
There are other AJAX functions in JQuery. As always, [the ajax documentation](https://api.jquery.com/category/ajax/) is your best friend!
...
- CodePen also provides files that can be fetched with AJAX. Read more about it [Ajax-CodePen](https://blog.codepen.io/2013/09/23/ajax-codepen/). Do something cool!
